"""
Centralized Game Master State - Global History Tracking
...
class GameMasterState:
    ...
    def get_sheriff_stats(self) -> dict:
        ...
        bribe_acceptance_rate = bribes_accepted / bribes_offered if bribes_offered > 0 else 0.3
...
# Global game master state instance
_game_master_state: Optional[GameMasterState] = None


def get_game_master_state() -> GameMasterState:
    """Get or create the global game master state."""
    global _game_master_state
    if _game_master_state is None:
        _game_master_state = GameMasterState()
    return _game_master_state
