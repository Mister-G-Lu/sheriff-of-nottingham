...
def update_sheriff_reputation(
    sheriff: Sheriff,
    inspected: bool,
    is_honest: bool,
    stats: GameStats = None,
    actual_goods: list[Good] = None,
) -> None:
    """
    ...
    if inspected and not is_honest:
        ...
        if actual_goods:
            ...
            if has_contraband:
                ...
                sheriff.level_up(1)
                print(
                    "A clever merchant tricked you, but no contraband passed. You learn from this."
                )
        else:
    ...
    print(
        f"[Sheriff] Reputation: {sheriff.reputation}/10  |  Experience: {sheriff.experience}"
    )
