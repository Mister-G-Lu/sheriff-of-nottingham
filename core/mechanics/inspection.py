...
from core.game.game_rules import calculate_confiscation_penalty, separate_declared_and_undeclared


def handle_inspection(merchant, actual_goods: list[Good], declaration: dict, sheriff) -> dict:
    """
    ...
    is_fully_honest = (
        len(actual_goods) == declared_count and
        all(g.id == declared_id for g in actual_goods)
    )
...
def handle_pass_without_inspection(merchant, actual_goods: list[Good], declaration: dict) -> dict:
    """
