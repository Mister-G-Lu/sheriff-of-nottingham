...
def redraw_cards(
    current_hand: list[Good],
    num_to_redraw: int,
    prefer_contraband: bool = False,
    ...
    if prefer_contraband:
        ...
        sorted_hand = sorted(
            current_hand, key=lambda g: (not g.is_contraband(), -g.value)
        )
    elif prefer_legal:
...
def should_redraw_for_better_hand(
    hand: list[Good], risk_tolerance: int, honesty: int, greed: int = 5
) -> int:
    """
    ...
    # GREEDY SMUGGLERS: Check for contraband set bonus opportunity
    # If merchant has 1-2 of the same contraband, they might redraw to complete the set
    from core.mechanics.contraband_bonus import should_redraw_for_contraband_set

    should_redraw_set, num_for_set, target_contraband = (
        should_redraw_for_contraband_set(hand, risk_tolerance, greed)
    )

    if should_redraw_set and num_for_set > 0:
        # Greedy merchant wants to complete contraband set for bonus multiplier!
        return num_for_set

    # AGGRESSIVE SMUGGLERS: Want contraband (general strategy)
...
def select_from_hand(
    hand: list[Good], desired_ids: list[str], max_count: int = 6
) -> list[Good]:
    """
...
def get_best_available_substitute(
    hand: list[Good], desired_id: str, is_legal: bool = None
) -> Good:
    """
