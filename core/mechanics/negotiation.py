...
def initiate_threat(sheriff: Sheriff, merchant: Merchant, goods_value: int) -> NegotiationState:
...
def sheriff_respond_to_bribe(negotiation: NegotiationState, player_choice: str, player_demand: int = 0) -> bool:
...
def merchant_respond_to_counter(negotiation: NegotiationState) -> tuple[bool, int]:
    ...
    increment = int((sheriff_demand - last_offer) * (0.3 + greed_factor * 0.3))  # 30-60% of gap
