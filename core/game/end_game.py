...
def _create_result(lore: dict, lore_key: str, state: EndGameState, 
                   default_title: str, default_rating: str, 
                   default_text: list[str]) -> EndGameResult:
    """
...
def determine_end_game_state(sheriff, stats) -> EndGameResult:
    ...
    if rep >= 7 and gold >= 50 and caught >= 3:
        return _create_result(lore, "legendary_sheriff", EndGameState.LEGENDARY_SHERIFF,
                             "🏆 THE LEGENDARY SHERIFF", "★★★★★ LEGENDARY", ["Victory!"])
    
    elif rep >= 7 and bribes == 0:
        return _create_result(lore, "honorable_sheriff", EndGameState.HONORABLE_SHERIFF,
                             "⚔️ THE HONORABLE SHERIFF", "★★★★☆ HONORABLE", ["Victory!"])
    
    elif rep <= 3 and bribes >= 5 and gold >= 50:
        return _create_result(lore, "corrupt_baron", EndGameState.CORRUPT_BARON,
                             "💰 THE CORRUPT BARON", "★★★☆☆ CORRUPT", ["Victory!"])
    
    # Standard ratings based on reputation
    elif rep >= 7:
        return _create_result(lore, "excellent", EndGameState.EXCELLENT,
                             "Excellent Performance", "★★★★☆ EXCELLENT", ["Good job!"])
    
    elif rep >= 4:
        return _create_result(lore, "good", EndGameState.GOOD,
                             "Good Performance", "★★★☆☆ GOOD", ["Decent work."])
    
    elif rep >= 2:
        return _create_result(lore, "mediocre", EndGameState.MEDIOCRE,
                             "Mediocre Performance", "★★☆☆☆ MEDIOCRE", ["Could be better."])
    
    elif rep > 0:
        return _create_result(lore, "poor", EndGameState.POOR,
                             "Poor Performance", "★☆☆☆☆ POOR", ["Not good."])
    
    else:  # rep == 0
        return _create_result(lore, "fired", EndGameState.FIRED,
                             "💀 GAME OVER - FIRED", "☆☆☆☆☆ FIRED", ["Game Over."])


def show_end_game_summary(sheriff, stats, black_market=None):
    """
    ...
    Args:
        ...
        stats: GameStats object
        black_market: Optional BlackMarketContact object
    """
    print("\n" + "="*70)
    ...
    # Add black market note if deal was accepted
    if black_market and black_market.deal_accepted:
        print("⚠️  Note: You accepted the Black Market deal.")
        print("   Your soul may be forfeit, but your pockets are full.")
        print()
