...
def load_inspection_messages() -> dict:
    ...
    messages_path = (
        Path(__file__).parent.parent.parent / "data" / "inspection_messages.json"
    )
    try:
...
def show_inspection_header(
    merchant_name: str,
    declaration_count: int,
    declaration_good: str,
    messages: dict = None,
):
    ...
    decl_label = messages.get("declaration", {}).get(
        "label", "📋 DECLARED: {count}x {good_id}"
    )
...
def show_bag_contents(actual_goods: list[Good], messages: dict = None):
    ...
    item_format = bag_config.get(
        "item_format", "   {index}. {good_id} (value: {value}g){contraband_marker}"
    )
    ...
    for i, good in enumerate(actual_goods, 1):
        ...
        print(
            item_format.format(
                index=i, good_id=good.id, value=good.value, contraband_marker=marker
            )
        )


def show_bluff_succeeded(
    merchant_name: str, goods_value: int, merchant_gold: int, messages: dict = None
):
    ...
    sold_msg = financial.get(
        "merchant_sold_all",
        "💰 [{merchant_name}] Sold all goods for {value}g. Total: {total}g",
    )
    print(
...
def show_honest_verdict(
    ...
    title = honest_config.get(
        "title", "✅ VERDICT: HONEST - Declaration matches contents!"
    )
    ...
    financial = messages.get("financial", {})
    sold_msg = financial.get(
        "merchant_sold", "💰 [{merchant_name}] Sold goods for {value}g. Total: {total}g"
    )
    print(
        f"\n{sold_msg.format(merchant_name=merchant_name, value=goods_value, total=merchant_gold)}"
    )
...
def show_lying_verdict(
    goods_passed: list[Good],
    goods_confiscated: list[Good],
    penalty_paid: int,
    merchant_name: str,
    merchant_gold: int,
    messages: dict = None,
    ...
    title = lying_config.get(
        "title", "❌ VERDICT: LYING - Declaration does NOT match contents!"
    )
    print(f"\n{title}")
    ...
    if goods_passed:
        ...
        header = allowed_config.get(
            "header", "✓ ALLOWED THROUGH ({count} items - truthfully declared):"
        )
        item_format = allowed_config.get(
            "item_format", "   • {good_id} (value: {value}g)"
        )
    ...
    if goods_confiscated:
        ...
        header = confiscated_config.get(
            "header", "⛔ CONFISCATED ({count} items - undeclared):"
        )
        item_format = confiscated_config.get(
            "item_format", "   • {good_id} (value: {value}g){contraband_marker}"
        )
        marker_legal = confiscated_config.get("contraband_marker_legal", " [LEGAL]")
        marker_illegal = confiscated_config.get(
            "contraband_marker_illegal", " [CONTRABAND]"
        )
        ...
        for good in goods_confiscated:
            ...
            print(
                item_format.format(
                    good_id=good.id, value=good.value, contraband_marker=marker
                )
            )

        # Show penalty
        penalty_msg = financial.get(
            "penalty",
            "💸 PENALTY: {penalty}g (50% of {total_value}g confiscated value)",
        )
        print(
            f"\n{penalty_msg.format(penalty=penalty_paid, total_value=total_confiscated_value)}"
        )

        remaining_msg = financial.get(
            "merchant_remaining", "   [{merchant_name}] Gold remaining: {gold}g"
        )
...
def show_tell(tell: str, messages: dict = None):
    ...
    if tell:
        ...
        tell_format = messages.get("tells", {}).get(
            "notice_format", "You notice: {tell}."
        )
        print(tell_format.format(tell=tell))


def show_bribe_accepted_status(
    amount: int, sheriff_reputation: int, messages: dict = None
):
    """Display sheriff status after accepting bribe."""
    if messages is None:
        messages = load_inspection_messages()

    status_msg = messages.get("sheriff_status", {}).get(
        "bribe_accepted",
...
def show_merchant_sold_goods(
    ...
    sold_msg = messages.get("financial", {}).get(
        "merchant_sold", "💰 [{merchant_name}] Sold goods for {value}g. Total: {total}g"
    )
    print(
        sold_msg.format(
            merchant_name=merchant_name, value=goods_value, total=merchant_gold
        )
    )
