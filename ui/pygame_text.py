...
class PygameText:
    ...
    def __init__(self, window):
        ...
        self.text_speed_wpm: int = TEXT_SPEED_WPM
        self.text_display_complete: bool = True

        # Fullscreen mode for special screens like "How to Play"
        self.fullscreen_mode: bool = False
        self.normal_text_box_y = TEXT_BOX_Y
        self.normal_text_box_height = TEXT_BOX_HEIGHT

    def set_fullscreen_mode(self, enabled: bool):
        """Enable/disable fullscreen text mode (expands text zone to full screen)"""
        self.fullscreen_mode = enabled

    def display_static_text(self, lines: list[str]):
        """Display all text at once as static content (no typewriter effect).

        This is used for screens like "How to Play" where all content should
        be visible immediately and scrollable.
        ...
        # Add all lines at once (no animation)
        for line in lines:
            # Wrap each line if needed
            wrapped = self.wrap_text(
                line, TEXT_BOX_WIDTH - TEXT_WRAP_MARGIN, self.window.font_normal
            )
            self.text_lines.extend(wrapped)
    ...
    def display_text(
        ...
        for line in lines:
            if line.strip():  # Non-empty line
                ...
                wrapped = self.wrap_text(
                    line, TEXT_BOX_WIDTH - 40, self.window.font_normal
                )
    ...
    def render(self):
        ...
        try:
            ...
            # Stats bar/price menu not initialized yet - skip rendering
            # This is expected during initialization
            pass

        # Adjust text box dimensions for fullscreen mode
        if self.fullscreen_mode:
            text_box_y = 0
            text_box_height = self.window.screen.get_height()
        else:
        ...
        y_offset = text_box_y + TEXT_BOX_VERTICAL_PADDING
        max_lines = (text_box_height - TEXT_BOX_PADDING) // (
            FONT_SIZE_NORMAL + LINE_SPACING
        )
        ...
        for line in visible_lines:
            ...
            self.window.screen.blit(
                text_surface, (TEXT_BOX_X + TEXT_BOX_VERTICAL_PADDING, y_offset)
            )
            y_offset += FONT_SIZE_NORMAL + LINE_SPACING

        # Draw scroll indicator if there's more content
        if len(self.text_lines) > max_lines:
            scroll_text = (
                f"[Scroll: {start_line + 1}-{end_line} of {len(self.text_lines)} lines]"
            )
            scroll_surface = self.window.font_small.render(scroll_text, True, GRAY)
            self.window.screen.blit(
                scroll_surface,
                (TEXT_BOX_X + TEXT_BOX_WIDTH - SCROLL_INDICATOR_WIDTH, text_box_y + 5),
            )
        ...
        if not self.text_display_complete:
            ...
            indicator_surface = self.window.font_small.render(
                indicator_text, True, GREEN
            )
            self.window.screen.blit(
                indicator_surface,
                (TEXT_BOX_X + TEXT_BOX_VERTICAL_PADDING, text_box_y + 5),
            )
