...
def test_roll_bluff_monkeypatched():
    ...
    assert 1 <= result <= 10  # Should be in valid range


def test_load_portrait_missing():
    m = Merchant(id="x", name="X", intro="Test merchant", tells_honest=[], tells_lying=[])
    # If portrait file is missing, load_portrait returns None or empty
    portrait = load_portrait(m)
    # We just test it doesn't crash
    assert portrait is not None or portrait is None  # trivial, but shows no exceptions
