...
from core.mechanics.goods import PEPPER, SILK
...
def test_bold_merchant():
    ...
    if wants_to_negotiate:
        ...
        print(
            f"  (Bold/greedy merchants offer less: ~{int(offer / goods_value * 100)}% of contraband value)"
        )
    else:
        print(
            "✗ Merchant refuses to bribe (bold merchants sometimes gamble on not bribing)"
        )
...
def test_merchant_giving_up():
    ...
    print(
        f"Sheriff demands: {unreasonable_demand} gold (more than contraband is worth!)"
    )
