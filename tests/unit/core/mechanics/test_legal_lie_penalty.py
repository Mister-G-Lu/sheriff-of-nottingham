...
from core.mechanics.goods import BREAD, CHEESE, CHICKEN
    ...
    def test_legal_lie_caught_pays_penalty(self):
        ...
        for _ in range(10):
            ...
            if result["caught_lie"]:
                ...
                assert not result["was_honest"], "Should not be honest"
                assert result["caught_lie"], "Should catch the lie"
                assert len(result["goods_confiscated"]) == 3, (
                    "Should confiscate all 3 chickens"
                )
                assert len(result["goods_passed"]) == 0, (
                    "Should pass 0 goods (no cheese found)"
                )
        ...
        assert caught_at_least_once, (
            "Sheriff should catch the lie at least once in 10 attempts"
        )
    ...
    def test_legal_lie_not_caught_no_penalty(self):
        ...
        for _ in range(10):
            ...
            if not result["caught_lie"]:
                ...
                assert not result["was_honest"], "Should not be honest"
                assert not result["caught_lie"], "Should not catch the lie"
        ...
        assert missed_at_least_once, (
            "Sheriff should miss the lie at least once in 10 attempts"
        )
    ...
    def test_partial_legal_lie_penalty(self):
        ...
        for _ in range(10):
            ...
            if result["caught_lie"]:
                ...
                assert len(result["goods_confiscated"]) == 2, (
                    "2 chickens should be confiscated"
                )
                assert result["penalty_paid"] == expected_penalty, (
    ...
    def test_mixed_legal_goods_different_values(self):
        ...
        for _ in range(10):
            ...
            if result["caught_lie"]:
                ...
                assert len(result["goods_confiscated"]) == 4, (
                    "All goods should be confiscated"
                )
                assert result["penalty_paid"] == expected_penalty, (
                    f"Should pay {expected_penalty}g penalty"
                )
