...
from core.mechanics.deck import (
    ...
    draw_hand,
    redraw_cards,
    should_redraw_for_contraband,
)
from core.mechanics.goods import APPLE, CHEESE, CHICKEN, CROSSBOW, PEPPER, SILK
...
class TestRedrawCards:
    ...
    def test_redraw_invalid_count(self):
        ...
        assert new_hand == original_hand, (
            "Should return original hand for invalid count"
        )

        # Negative redraw
        new_hand = redraw_cards(original_hand, -1)
        assert new_hand == original_hand, (
            "Should return original hand for negative count"
        )
...
class TestShouldRedrawForContraband:
    ...
    def test_honest_merchant_redraws_for_consistency(self):
        ...
        assert num_to_redraw >= 1, (
            "Honest merchants should redraw to get more of the same good"
        )
    ...
    def test_aggressive_smuggler_redraws_with_no_contraband(self):
        ...
        assert num_to_redraw >= 3, (
            "Aggressive smugglers should redraw 3-4 cards with no contraband"
        )
...
class TestRedrawIntegration:
    ...
    def test_aggressive_merchant_gets_more_contraband_opportunities(self):
        ...
        for _ in range(50):
            ...
            hand = draw_hand(7)
            num_to_redraw = should_redraw_for_contraband(
                hand, risk_tolerance=10, honesty=1
            )
            if num_to_redraw > 0:
        ...
        avg_with_redraw = sum(contraband_counts_with_redraw) / len(
            contraband_counts_with_redraw
        )
        avg_without_redraw = sum(contraband_counts_without_redraw) / len(
            contraband_counts_without_redraw
        )
