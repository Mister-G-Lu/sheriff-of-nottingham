...
def test_bribe_scaling_examples():
    ...
    ratio_very_high = bribe_very_high / declared_value_very_high if declared_value_very_high > 0 else 0
    ...
    print(f"✅ STRATEGIC! High declaration justifies high bribe")
    ...
    print(f"High declaration (35g): Bribe = {bribe_very_high}g  ({ratio_very_high:.1f}x)")
...
def test_merchant_rationality():
    ...
    if bribe < contraband_value:
        ...
        print(f"   Merchant won't pay more than goods are worth")
    else:
        print(f"❌ IRRATIONAL! Merchant would lose money")
    ...
    print(f"Actually carrying: Low contraband (worth {contraband_value}g)")
    print(f"Bribe offered: {bribe}g")
    print(f"✅ Minimum bribe ensures merchant still offers something")
...
def test_personality_effects():
    ...
    print(f"   - Greedy merchants offer less")
    print(f"   - Risk-takers gamble with lower bribes")
