...
from core.systems.game_master_state import (
    GameMasterState, MerchantTier, get_game_master_state, reset_game_master_state
)
from ai_strategy.bribe_strategy import (
    calculate_scaled_bribe, calculate_declared_value, calculate_contraband_value
)
...
def test_game_master_state():
    ...
    print(f"✓ Sheriff stats: inspection_rate={stats['inspection_rate']:.2f}, catch_rate={stats['catch_rate']:.2f}")
...
def test_bribe_scaling():
    ...
    sheriff_stats = {'inspection_rate': 0.5, 'catch_rate': 0.5, 'bribe_acceptance_rate': 0.3}
    ...
    print(f"Scenario 1: Declare 1x Cheese ({declared_value_low}g) with contraband ({contraband_value}g)")
    ...
    print(f"✓ Prevents obvious tells (low declaration + high bribe)")
...
def test_tiered_behavior():
    ...
    for tier in [MerchantTier.EASY, MerchantTier.MEDIUM, MerchantTier.HARD]:
        ...
        print(f"  Honest personality → Strategy: {decision.get('strategy', 'N/A')}, Lie: {decision['lie']}")
        
        # Risky personality
        risky_personality = {'risk_tolerance': 8, 'greed': 8, 'honesty_bias': 2}
        decision = get_tiered_declaration(risky_personality, tier)
        print(f"  Risky personality → Strategy: {decision.get('strategy', 'N/A')}, Lie: {decision['lie']}")
...
def test_personality_modifiers():
    ...
    print(f"Honest merchant (honesty=9):")
    print(f"  → Strategy: {honest_decision.get('strategy', 'N/A')}, Lie: {honest_decision['lie']}")
    
    print(f"\nRisky merchant (risk=9, greed=9, honesty=1):")
    print(f"  → Strategy: {risky_decision.get('strategy', 'N/A')}, Lie: {risky_decision['lie']}")
