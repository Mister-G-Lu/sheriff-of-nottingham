...
class TestRiskScoreCalculation:
    ...
    def test_easy_tier_honest_merchant(self):
        ...
        risk_score = TieredMerchantStrategy._calculate_risk_score(
            honesty=10, risk=0, greed=0,
            inspection_rate=0.5, catch_rate=0.5,
    ...
    def test_medium_tier_greedy_merchant(self):
        ...
        risk_score_low_greed = TieredMerchantStrategy._calculate_risk_score(
            honesty=5, risk=5, greed=0,
            inspection_rate=0.5, catch_rate=0.5,
            tier=MerchantTier.MEDIUM
        )
        risk_score_high_greed = TieredMerchantStrategy._calculate_risk_score(
            honesty=5, risk=5, greed=10,
            inspection_rate=0.5, catch_rate=0.5,
            tier=MerchantTier.MEDIUM
        )
        assert risk_score_high_greed > risk_score_low_greed, "High greed should increase risk score"
    ...
    def test_sheriff_aggression_reduces_risk(self):
        ...
        assert risk_score_aggressive < risk_score_lenient, "Aggressive sheriff should reduce risk score"
    ...
    def test_hard_tier_pattern_exploitation(self):
        ...
        assert risk_score_with_pattern > risk_score_without_pattern, "Hard tier should exploit lenient patterns"
...
class TestStrategyWeights:
    ...
    def test_medium_tier_more_aggressive_than_easy(self):
        ...
        assert medium_weights[0] < easy_weights[0], "Medium should be less honest than Easy"
        assert medium_weights[3] + medium_weights[4] > easy_weights[3] + easy_weights[4], \
...
class TestStrategySelection:
    ...
    def test_honest_merchant_mostly_honest_strategy(self):
        ...
        assert honest_count >= 80, f"Honest merchant should be honest 80%+ of the time, got {honest_count}%"
    ...
    def test_bold_merchant_mostly_contraband_strategy(self):
        ...
        # Bold merchants should smuggle more than 50% of the time
        assert contraband_count >= 50, f"Bold merchant should smuggle 50%+ of the time, got {contraband_count}%"
        ...
        assert contraband_count > honest_contraband_count * 3, \
            f"Bold merchant should smuggle much more than honest merchant"
    ...
    def test_hard_tier_reverse_psychology(self):
        ...
        assert honest_count >= 15, "Hard tier should use reverse psychology against effective sheriffs"
