...
def test_resolve_inspection_opens_and_catches(monkeypatch):
    ...
    m = Merchant(id="m", name="M", intro="Test merchant", tells_honest=[], tells_lying=[], bluff_skill=1)
    decl = Declaration(good_id="apple", count=2)
    actual = [SILK]
...
def test_resolve_inspection_records_contraband_when_slips(monkeypatch):
    ...
    m = Merchant(id="m", name="M", intro="Test merchant", tells_honest=[], tells_lying=[], bluff_skill=10)
