...
def run_coverage():
    ...
    if good_coverage:
        print("✅ Files with >= 80% coverage:")
        print()
        for name, _stmts, _miss, cover in sorted(
            good_coverage, key=lambda x: x[3], reverse=True
        ):
    ...
    overall_coverage = (
        int((total_stmts - total_miss) / total_stmts * 100) if total_stmts > 0 else 0
    )
    ...
    if overall_coverage >= 80:
        ...
        print(
            f"   Need to cover {int((80 - overall_coverage) * total_stmts / 100)} more statements"
        )
